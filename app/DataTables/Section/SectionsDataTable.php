...
class SectionsDataTable extends DataTable
{
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
                ...
                $query->where(function ($query) use ($keyword) {
                    ...
                    foreach ($words as $word) {
                        $query->orWhere(function ($query) use ($word) {
                            if (str_contains('%active%', strtolower($word)) || str_contains('%نشط%', $word)) {
                                $query->where('is_active', 1);
                            }
                            elseif (str_contains('%inactive%', strtolower($word)) || str_contains('%غير%', $word)) {
            ...
            ->rawColumns(['checkbox','actions', 'title_en', 'title_ar','is_active'])
            ->setRowId('id');
    }

    protected function renderActions($row): string
    {
        $editUrl = route('sections.edit', $row->id);
        $routeName = 'sections';
        $modelName = 'sections';
        $hasDelete = 'no';
    ...
    public function query(CompanySection $model): QueryBuilder
    {
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('key')->title(__('dash.key')),
            Column::make('sort_order')->title(__('dash.sort_order')),
            Column::make('is_active')->title(__('dash.status'))->addClass('status-cell'),
