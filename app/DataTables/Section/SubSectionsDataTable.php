...
class SubSectionsDataTable extends DataTable
{
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('section', fn($row) => $row->section ? $row->section->title : '')
            ...
            ->rawColumns(['checkbox','actions', 'title_en', 'title_ar', 'section'])
            ->setRowId('id');
    }

    protected function renderActions($row): string
    {
        $editUrl = route('sub-sections.edit', $row->id);
        $routeName = 'sub-sections';
        $modelName = 'sub_sections';
        return view('components.dashboard.partials.actions_dropdown', compact('editUrl','routeName','modelName'))->render();
    }

    public function query(CompanySubSection $model): QueryBuilder
    {
        return $model->newQuery()->with('section');
    }

    public function html(): HtmlBuilder{
        return $this->builder()
            ->setTableId('sub-sections-table')
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('section')->title(__('dash.section'))->name('section.title'),
    ...
    {
        return 'SubSections_' . date('YmdHis');
    }
}
