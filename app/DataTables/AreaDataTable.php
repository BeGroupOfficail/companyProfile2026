...
class AreaDataTable extends DataTable
{
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('region', fn($row) => $row->region->getTranslation('name','ar') ?? '')
            ->addColumn('country', fn($row) => $row->region->country->getTranslation('name','ar') ?? '')
    ...
    {
        $editUrl = route('settings.areas.edit', $row->id);
        $routeName = 'settings.areas';
        $modelName = 'areas';
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('region')->title(__('dash.region')),
