...
class TourDataTable extends DataTable
{
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('tour_type', fn($row) => $row->tour_type ?? '')
            ->addColumn('person_price_per_day', fn($row) => $row->person_price_per_day ?? '')
            ->addColumn('number_days', fn($row) => $row->number_days ?? '')
            ->addColumn('number_nights', fn($row) => $row->number_nights ?? '')

            // Render image columns directly (e.g., image, icon)
            ->addColumn('image', function($row){
                return $this->renderImage($row->image, 'tours', 100);
            })
            ...
            ->rawColumns(['checkbox','actions', 'name_en', 'name_ar','tour_type', 'person_price_per_day', 'number_days', 'number_nights','image','status'])
            ->setRowId('id');
    }
    ...
    {
        $editUrl = route('tours.edit', $row->id);
        $routeName = 'tours';
        $modelName = 'tours';
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('tour_type')->title(__('dash.tour_type')),
            Column::make('person_price_per_day')->title(__('dash.person_price_per_day')),
            Column::make('number_days')->title(__('dash.number_days')),
            Column::make('number_nights')->title(__('dash.number_nights')),
