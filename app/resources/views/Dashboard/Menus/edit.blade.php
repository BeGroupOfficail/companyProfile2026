<x-dashboard.layout :title="__('dash.edit_menu')">

    <!--begin::Form-->
    <form method="post" action="{{route('menus.update',$menu->id)}}" class="form d-flex flex-column flex-lg-row" data-kt-redirect="{{route('menus.index')}}" enctype="multipart/form-data">
        @csrf
        @method('patch')
        <!--begin::Aside column-->
        ...
        <div class="d-flex flex-column flex-row-fluid gap-7 gap-lg-10">
                                ...
                                <div class="d-flex flex-wrap gap-5">
                                    @foreach(config('languages') as $lang => $languageName)
                                        <x-dashboard.partials.html.input
                                            ...
                                            :value="old('name_' . $lang, $menu->getTranslation('name', $lang) ?? '')"
            ...
            <div class="d-flex justify-content-end">
                <!--begin::Button-->
                <a href="{{route('menus.index')}}" id="kt_ecommerce_add_product_cancel" class="btn btn-light me-5">{{__('dash.Cancel')}}</a>
                <!--end::Button-->
