<x-dashboard.layout :title="__('dash.videos')">
    ...
    <!--begin::Form-->
    <form method="POST" action="{{ route('videos.update', $video->id) }}" class="form d-flex flex-column flex-lg-row"
                                ...
                                <div class="d-flex flex-wrap gap-5">
                                    <div class="row fv-row mb-7">
                                        <x-dashboard.partials.html.input name="video" :required="'required'"
                                            :value="old('video', $video->video)" label="{{ __('dash.video') }}" placeholder="video link" />
                                        <x-dashboard.partials.html.image_input :title="'Upload Image'" :name="'image'"
                                            :model="$video" :imageModelName="'videos'" :description="'Only *.png, *.jpg, and *.jpeg image files are accepted.'" :changeImageText="'Change Image'"
                                            :cancelImageText="'Cancel Image'" :removeImageText="'Remove Image'" :acceptedText="'image files are accepted'" />
