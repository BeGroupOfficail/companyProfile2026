<x-dashboard.layout :title="__('dash.edit_slider')">

    <!--begin::Form-->
    <form method="POST" action="{{route('sliders.update',$slider->id)}}" class="form d-flex flex-column flex-lg-row" data-kt-redirect="{{route('sliders.index')}}" enctype="multipart/form-data">
        @csrf
        @method('PATCH')
        <!--begin::Aside column-->
        <div class="d-flex flex-column gap-7 gap-lg-10 w-100 w-lg-300px mb-7 me-lg-10">

            <!--begin::icon-->
            <x-dashboard.partials.html.image_input
                :title="'Upload image'"
                :name="'image'"
                :description="'Only *.png, *.jpg, and *.jpeg icon files are accepted.'"
                :changeImageText="'Change image'"
                :cancelImageText="'Cancel image'"
                :removeImageText="'Remove image'"
                :acceptedText="'icon files are accepted'"
                :model="$slider"
                :imageModelName="'sliders'"
            />
            <!--end::Icon -->

            <!--begin::Status-->
            <x-dashboard.partials.html.status_select
                :model="'slider'"
                :selected="$slider->status"
                :modelObject="$slider"
            />
            <!--end::Status-->
        </div>
        <!--end::Aside column-->

        <!--begin::Main column-->
        <div class="d-flex flex-column flex-row-fluid gap-7 gap-lg-10">

            <!--begin::Tab content-->
            <div class="tab-content">
                <!--begin::Tab pane-->
                <div class="tab-pane fade show active" id="tab1" role="tab-panel">
                    <div class="d-flex flex-column gap-7 gap-lg-10">

                        <!--begin::General options-->
                        <div class="card card-flush py-4">

                            <!--begin::Card body-->
                            <div class="card-body pt-0">

                                <div class="d-flex flex-wrap gap-5">
                                    <x-dashboard.partials.html.input
                                        name="title"
                                        label="{{ __('dash.title') }}"
                                        :value="old('title', $slider->title ?? '')"
                                        placeholder="{{ __('dash.Enter the title') }}" />

                                </div>

                                <div class="d-flex flex-wrap gap-5">
                                    <x-dashboard.partials.html.textarea
                                        name="text"
                                        label="{{ __('dash.desc') }}"
                                        :value="old('text', $slider->text ?? '')"
                                        placeholder="{{ __('dash.Enter the text') }}" />
                                </div>

                                <div class="d-flex flex-wrap gap-5">
                                    <x-dashboard.partials.html.input
                                        name="link"
                                        label="{{ __('dash.link') }}"
                                        :value="old('link', '')"
                                        placeholder="{{ __('dash.link') }}" />

                                    <x-dashboard.partials.html.select
                                        :options="config('languages')"
                                        :name="'lang'"
                                        :selectedValue="$slider->lang "
                                        :title="__('dash.lang')"
                                        :id="'lang'" />

                                    <x-dashboard.partials.html.input
                                        name="order"
                                        type="number"
                                        label="{{ __('dash.order') }}"
                                        :value="old('order', $slider->order ?? '')"
                                        placeholder="{{ __('dash.order') }}" />
                                </div>

                            </div>
                            <!--end::Card header-->

                        </div>
                        <!--end::General options-->

                    </div>
                </div>
                <!--end::Tab pane-->

            </div>
            <!--end::Tab content-->

            <div class="d-flex justify-content-end">
                <!--begin::Button-->
                <a href="{{route('sliders.index')}}" id="kt_ecommerce_add_product_cancel" class="btn btn-light me-5">{{__('dash.Cancel')}}</a>
                <!--end::Button-->

                <!--begin::Button-->
                <button type="submit" id="kt_ecommerce_add_product_submit" class="btn btn-primary">
                    <span class="indicator-label">{{__('dash.Save Changes')}}</span>
                    <span class="indicator-progress">{{__('dash.Please wait...')}} <span class="spinner-border spinner-border-sm align-middle ms-2"></span>
                 </span>
                </button>
                <!--end::Button-->
            </div>
        </div>
        <!--end::Main column-->
    </form>
    <!--end::Form-->

</x-dashboard.layout>
