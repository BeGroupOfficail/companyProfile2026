    ...
    <form method="POST" action="{{route('categories.update',$category->id)}}" class="form d-flex flex-column flex-lg-row" data-kt-redirect="{{route('categories.index')}}" enctype="multipart/form-data">
            ...
            {{-- <div class="card card-flush py-4">
                ...
                <div class="card-body pt-0">
                    ...
                    <select class="form-select mb-2" data-control="select2" data-hide-search="true"
                        data-placeholder="Select an option" id="kt_ecommerce_add_product_status_select" name="job_seeker">
                        <option></option>
                        <option @selected(old('job_seeker',$field->job_seeker) == 1) value="1">{{ __('dash.yes') }}</option>
                        <option @selected(old('job_seeker',$field->job_seeker) == 0) value="0">{{ __('dash.no') }}</option>
                    </select>
                    <!--end::Select2-->

                    <!--begin::Description-->
                    <div class="text-muted fs-7">{{ __('dash.Set the') }} {{ _('dash.field') }} {{ __('dash.job_seeker') }}.
                    </div>
                    <!--end::Description-->
                </div>
                <!--end::Card body-->
        ...
        <div class="d-flex flex-column flex-row-fluid gap-7 gap-lg-10">
                            ...
                            <div class="card-body pt-0">
                                ...
                                <div class="d-flex flex-wrap gap-5">
                                    <x-dashboard.partials.html.objects-select
                                        name="parent_id"
                                        title="{{ __('dash.parent_category') }}"
                                        :options="$categories"
                                        :isTranslatable="true"
                                        :translatableValue="'name'"
                                        :selectedValue="$category->parent_id"/>

                                    <x-dashboard.partials.html.input
                                        name="shortcut"
                                        label="{{ __('dash.shortcut') }}"
                                        :value="old('shortcut', $category->shortcut ?? '')"
                                        placeholder="{{ __('dash.Enter the shortcut') }}" />
                                </div>
                                ...
                                <div class="d-flex flex-wrap gap-5">
                                    <x-dashboard.partials.html.select
                                        :options="$fields"
                                        :selectedArray="old('fields', $selectedField)"
                                        :name="'fields[]'"
                                        :title="__('dash.fields')"
                                        :id="'fields-select'"
                                        :isMultiple="true" />
                                </div>
                                ...
                                <div class="d-flex flex-wrap gap-5">
                                    @foreach(config('languages') as $lang => $languageName)
                                        <x-dashboard.partials.html.textarea
                                            ...
                                            :value="old('short_desc_' . $lang, $category->getTranslation('short_desc', $lang) ?? '')"
                                ...
                                @foreach(config('languages') as $lang => $languageName)
                                    <x-dashboard.partials.html.textarea_with_editor
                                        ...
                                        :value="old('long_desc_' . $lang, $category->getTranslation('long_desc', $lang) ?? '')"
                                    ...
                                    <div class="row fv-row mb-7">
                                        ...
                                        <div class="col-md-6">
                                            <div class="d-flex mt-3">
                                                <!--begin::Radio-->
                                                <div class="form-check form-check-custom form-check-solid me-5">
                                                    <input class="form-check-input" type="radio" value="1" name="menu" @checked($category->menu == 1)>
                                                    <label class="form-check-label" for="header_yes">@lang('dash.yes')</label>
                                                </div>

                                                <div class="form-check form-check-custom form-check-solid">
                                                    <input class="form-check-input" type="radio" value="0" name="menu" @checked($category->menu == 0)>
                                ...
                                <div class="d-flex flex-wrap gap-5">
                                    @foreach(config('languages') as $lang => $languageName)
                                        <x-dashboard.partials.html.input
                                            ...
                                            :value="old('slug_' . $lang, $category->getTranslation('slug', $lang) ?? '')"
                                ...
                                <div class="d-flex flex-wrap gap-5">
                                    @foreach(config('languages') as $lang => $languageName)
                                        <x-dashboard.partials.html.input
                                            ...
                                            :value="old('meta_title_' . $lang, $category->getTranslation('meta_title', $lang) ?? '')"
                                ...
                                <div class="d-flex flex-wrap gap-5">
                                    @foreach(config('languages') as $lang => $languageName)
                                        <x-dashboard.partials.html.textarea
                                            ...
                                            :value="old('meta_desc_' . $lang, $category->getTranslation('meta_desc', $lang) ?? '')"
            ...
            <div class="d-flex justify-content-end">
                <!--begin::Button-->
                <a href="{{route('categories.index')}}" id="kt_ecommerce_add_product_cancel" class="btn btn-light me-5">{{__('dash.Cancel')}}</a>
                <!--end::Button-->
