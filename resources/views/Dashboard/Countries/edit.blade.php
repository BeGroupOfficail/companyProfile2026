<x-dashboard.layout :title="__('dash.edit_country')">

    <!--begin::Form-->
    <form method="POST" action="{{route('settings.countries.update',$country->id)}}" class="form d-flex flex-column flex-lg-row">
        @csrf
        @method('PATCH')
                                ...
                                <div class="d-flex flex-wrap gap-5">
                                    @foreach(config('languages') as $lang => $languageName)
                                        <x-dashboard.partials.html.input
                                            ...
                                            :value="old('name_' . $lang, $country->getTranslation('name', $lang) ?? '')"
