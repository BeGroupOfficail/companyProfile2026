<x-dashboard.layout :title="__('dash.edit_sub_section_item')">

    <!--begin::Form-->
    <form method="post" action="{{route('sub-section-items.update', $subSectionItem->id)}}" class="form d-flex flex-column flex-lg-row" data-kt-redirect="{{route('sub-section-items.index')}}" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <!--begin::Aside column-->
                ...
                <div class="card-body pt-0">
                    <select class="form-select mb-2" data-control="select2" name="sub_section_id" data-placeholder="{{ __('dash.Select an option') }}">
                        @foreach($subSections as $subSection)
                            <option value="{{ $subSection->id }}" {{ $subSectionItem->sub_section_id == $subSection->id ? 'selected' : '' }}>
                            ...
                            <div class="card-body pt-0">
                                ...
                                <div class="d-flex flex-wrap gap-5">
                                    <x-dashboard.partials.html.input
                                        ...
                                        :value="old('sort_order', $subSectionItem->sort_order)"
                                        placeholder="0" />
                                </div>
                                ...
                                <div class="d-flex flex-wrap gap-5 mt-5">
                                    @foreach(config('languages') as $lang => $languageName)
                                        <x-dashboard.partials.html.input
                                            ...
                                            :value="old('title_' . $lang, $subSectionItem->getTranslation('title', $lang))"
                                ...
                                <div class="mt-5">
                                    @foreach(config('languages') as $lang => $languageName)
                                        ...
                                        <textarea class="form-control mb-5" name="description_{{ $lang }}">{{ old('description_' . $lang, $subSectionItem->getTranslation('description', $lang)) }}</textarea>
                                    @endforeach
                                </div>
