<x-dashboard.layout :title="__('dash.edit_clientss')">
    ...
    <!--begin::Form-->
    <form method="POST" action="{{route('clients.update',$client->id)}}" class="form d-flex flex-column flex-lg-row" data-kt-redirect="{{route('clients.index')}}" enctype="multipart/form-data">
        @csrf
        @method('PATCH')
        <!--begin::Aside column-->
                                ...
                                <div class="d-flex flex-wrap gap-5">
                                    @foreach(config('languages') as $lang => $languageName)
                                        <x-dashboard.partials.html.textarea
                                            ...
                                            :value="old('desc_' . $lang, $client->getTranslation('desc', $lang) ?? '')"
