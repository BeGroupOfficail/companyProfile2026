<x-dashboard.layout :title="__('dash.edit_sub_section')">

    <!--begin::Form-->
    <form method="post" action="{{route('sub-sections.update', $subSection->id)}}" class="form d-flex flex-column flex-lg-row" data-kt-redirect="{{route('sub-sections.index')}}" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <!--begin::Aside column-->
            ...
            <div class="card card-flush py-4">
                ...
                <div class="card-body pt-0">
                    <select class="form-select mb-2" data-control="select2" data-hide-search="true" data-placeholder="{{ __('dash.Select an option') }}" name="layout">
                        <option value="title_desc" {{ $subSection->layout == 'title_desc' ? 'selected' : '' }}>{{ __('dash.title_desc') }}</option>
                        <option value="title_only" {{ $subSection->layout == 'title_only' ? 'selected' : '' }}>{{ __('dash.title_only') }}</option>
                    </select>
                </div>
            </div>
            <!--end::Layout-->
                            ...
                            <div class="card-body pt-0">
                                ...
                                <div class="d-flex flex-wrap gap-5">
                                    <x-dashboard.partials.html.input
                                        ...
                                        :value="old('sort_order', $subSection->sort_order)"
                                        placeholder="0" />
                                    <x-dashboard.partials.html.input
                                        name="key"
                                        type="text"
                                ...
                                <div class="d-flex flex-wrap gap-5 mt-5">
                                    @foreach(config('languages') as $lang => $languageName)
                                        <x-dashboard.partials.html.input
                                            ...
                                            :value="old('title_' . $lang, $subSection->getTranslation('title', $lang))"
                                ...
                                <div class="mt-5" id="description-section">
                                    @foreach(config('languages') as $lang => $languageName)
                                        ...
                                        <textarea class="form-control mb-5" name="description_{{ $lang }}">{{ old('description_' . $lang, $subSection->getTranslation('description', $lang)) }}</textarea>
                                    @endforeach
                                </div>
