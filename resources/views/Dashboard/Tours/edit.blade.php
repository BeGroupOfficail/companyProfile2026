<x-dashboard.layout :title="__('dash.edit_tour')">

    <!--begin::Form-->
    <form method="POST" action="{{route('tours.update',$tour->id)}}" class="form d-flex flex-column flex-lg-row" data-kt-redirect="{{route('tours.index')}}" enctype="multipart/form-data">
        @csrf
        @method('PATCH')
        <!--begin::Aside column-->
                            ...
                            <div class="card-body pt-0">
                                ...
                                <div class="d-flex flex-wrap gap-5">
                                    <x-dashboard.partials.html.select
                                        :options="$tourTypes"
                                        :name="'tour_type'"
                                        :title="__('dash.tour_type')"
                                        :id="'tour_type'"
                                        :selectedValue="$tour->tour_type" />

                                    <x-dashboard.partials.html.input
                                        name="number_days"
                                        type="number"
                                        label="{{ __('dash.number_days') }}"
                                        :value="$tour->number_days"
                                        placeholder="{{ __('dash.Enter number days') }}" />

                                    <x-dashboard.partials.html.input
                                        name="number_nights"
                                        type="number"
                                        label="{{ __('dash.number_nights') }}"
                                        :value="$tour->number_nights"
                                        placeholder="{{ __('dash.Enter number nights') }}" />

                                    <x-dashboard.partials.html.input
                                        name="person_price_per_day"
                                        type="number"
                                        label="{{ __('dash.person_price_per_day') }}"
                                        :value="$tour->person_price_per_day"
                                        placeholder="{{ __('dash.Enter person price per day') }}" />
                                ...
                                <div class="d-flex flex-wrap gap-5">
                                    @foreach(config('languages') as $lang => $languageName)
                                        <x-dashboard.partials.html.textarea
                                            ...
                                            :value="old('short_desc_' . $lang, $tour->getTranslation('short_desc', $lang) ?? '')"
                                ...
                                @foreach(config('languages') as $lang => $languageName)
                                    <x-dashboard.partials.html.textarea_with_editor
                                        ...
                                        :value="old('long_desc_' . $lang, $tour->getTranslation('long_desc', $lang) ?? '')"
                                        placeholder="{{ __('dash.Enter the long dec in') }} {{ __($languageName) }}" />
                                @endforeach

                                @foreach(config('languages') as $lang => $languageName)
                                    <x-dashboard.partials.html.textarea_with_editor
                                        name="tour_plan_{{ $lang }}"
                                        label="{{ __('dash.tour_plan') }} ({{ __($languageName) }})"
                                        :value="old('tour_plan_' . $lang, $tour->getTranslation('tour_plan', $lang) ?? '')"
                                        placeholder="{{ __('dash.Enter the tour plan in') }} {{ __($languageName) }}" />
                                @endforeach
                                        ...
                                        <div class="col-md-6">
                                            <div class="d-flex mt-3">
                                                ...
                                                <div class="form-check form-check-custom form-check-solid me-5">
                                                    <input class="form-check-input" type="radio" value="1" name="featured" @checked($tour->featured == 1) >
                                                    <label class="form-check-label" for="home_yes">@lang('dash.yes')</label>
                                                </div>

                                                <div class="form-check form-check-custom form-check-solid">
                                                    <input class="form-check-input" type="radio" value="0" name="featured" @checked($tour->featured == 0)>
                                    ...
                                    <div class="row fv-row mb-7">
                                        ...
                                        <div class="col-md-6">
                                            <div class="d-flex mt-3">
                                                <!--begin::Radio-->
                                                <div class="form-check form-check-custom form-check-solid me-5">
                                                    <input class="form-check-input" type="radio" value="1" name="home" @checked($tour->home == 1) >
                                                    <label class="form-check-label" for="home_yes">@lang('dash.yes')</label>
                                                </div>

                                                <div class="form-check form-check-custom form-check-solid">
                                                    <input class="form-check-input" type="radio" value="0" name="home" @checked($tour->home == 0)>
                                                    <label class="form-check-label" for="home_no">@lang('dash.no')</label>
                                                </div>
                                                <!--end::Radio-->
                                            </div>
                                    ...
                                    <div class="row fv-row mb-7">
                                        ...
                                        <div class="col-md-6">
                                            <div class="d-flex mt-3">
                                                <!--begin::Radio-->
                                                <div class="form-check form-check-custom form-check-solid me-5">
                                                    <input class="form-check-input" type="radio" value="1" name="menu" @checked($tour->menu == 1)>
                                                    <label class="form-check-label" for="header_yes">@lang('dash.yes')</label>
                                                </div>

                                                <div class="form-check form-check-custom form-check-solid">
                                                    <input class="form-check-input" type="radio" value="0" name="menu" @checked($tour->menu == 0)>
                                ...
                                <div class="d-flex flex-wrap gap-5">
                                    @foreach(config('languages') as $lang => $languageName)
                                        <x-dashboard.partials.html.input
                                            ...
                                            :value="old('slug_' . $lang, $tour->getTranslation('slug', $lang) ?? '')"
                                ...
                                <div class="d-flex flex-wrap gap-5">
                                    @foreach(config('languages') as $lang => $languageName)
                                        <x-dashboard.partials.html.input
                                            ...
                                            :value="old('meta_title_' . $lang, $tour->getTranslation('meta_title', $lang) ?? '')"
                                ...
                                <div class="d-flex flex-wrap gap-5">
                                    @foreach(config('languages') as $lang => $languageName)
                                        <x-dashboard.partials.html.textarea
                                            ...
                                            :value="old('meta_desc_' . $lang, $tour->getTranslation('meta_desc', $lang) ?? '')"
